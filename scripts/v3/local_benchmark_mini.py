...
Uses Unsloth direct inference path (no vLLM, no merge). Reads adapter
config from artifacts/v3/students/mini-local/. Loads base model (Gemma 4
E2B) automatically via Unsloth's LoRA-aware loader.

Output: artifacts/metrics/benchmark_mini.json (same schema as the OPF
3-way script — can be merged into the same reports).

Usage (inside Unsloth container):
    docker run --rm --gpus all --ipc=host --shm-size=8g \\
      -v $PWD:/workspace/gpf -v /tmp/hf-cache:/workspace/.cache/huggingface \\
      -e HF_HOME=/workspace/.cache/huggingface \\
      --entrypoint /bin/bash unsloth/unsloth:latest \\
      /workspace/gpf/scripts/v3/_run_in_container.sh \\
      /workspace/gpf/scripts/v3/local_benchmark_mini.py
"""
...
def main() -> None:
    ...
    print(f"[unsloth] loading {args.lora_adapter}", flush=True)
    from unsloth import FastLanguageModel  # type: ignore
    import torch

    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=str(args.lora_adapter),
    ...
    for i, case in enumerate(cases):
        msg = [
            ...
            {"role": "user",   "content": _to_blocks(case["text"])},
        ]
        ...
        if inputs.get("attention_mask") is not None and inputs["attention_mask"].dim() == 3:
        ...
        if (i + 1) % 10 == 0:
            ...
            print(f"[mini] {i+1}/{len(cases)} ({elapsed:.1f}s)", flush=True)
