#!/usr/bin/env bash
# ec2_v3_teacher.sh — STUB: forked from ec2_spot_finetune.sh.
#
# WARNING: this script is a launcher TEMPLATE for the v3 teacher SFT run
# (gemma-4-31B-it via Unsloth LoRA Q4). It still calls the v1/v2
# token-classifier training path. Before first use, replace the user-data
# block to:
#   1. pip install -r requirements-unsloth.txt
#   2. aws s3 sync data/processed/v3_chat/ /opt/gpf/data/v3_chat/
#   3. python scripts/v3/train_teacher.py --config configs/v3_distillation.yaml
#         --output-dir /opt/gpf/artifacts/v3/teacher/run-${TIMESTAMP}
#   4. aws s3 sync /opt/gpf/artifacts/v3/teacher/ s3://${BUCKET}/v3/teacher/
#
# Recommended instance: g6e.xlarge spot eu-north-1b ($0.63/h * 12 h = $7-8).
# See docs/V3_DISTILLATION_PLAN.md for full architecture.
#
