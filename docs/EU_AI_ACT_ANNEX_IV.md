...
| (h) Test logs and reports | v1 production fine-tune (run `20260426T135853Z`) produces baseline (`--eval-mode untyped`) and finetuned (`--eval-mode typed`) metrics on both the held-out `test.jsonl` (3,171 records) and the harder `hard_test.jsonl` (4,593 records). Files: `data/processed/aws-ft-20260426T135853Z/artifacts/metrics/baseline_test_metrics.json`, `baseline_hard_test_metrics.json`, `finetuned_test_metrics.json`, `finetuned_hard_test_metrics.json`. Headline span-F1: test 0.7766 → 0.9886 (+0.2120); hard_test 0.7668 → 0.9868 (+0.2200). Per-class breakdown in `docs/MODEL_CARD.md` §4. Earlier-iteration baselines against prior smoke splits (8,000 / 1,000 / 2,000 / 2,000 records) are retained unchanged as historical evidence in `artifacts/metrics/archive/`. |
